...
def _calc_partitions(df):
    ...
    # Calculate extra columns: if charge was to medicaid, and if charge was inpatient
    r_inpt = re.compile(r"medicaid", re.IGNORECASE)
    df["medicaid"] = df.insurance.apply(lambda x: bool(r_inpt.match(x)))
    ...
    partitions["wcc_encs"] = df[df.cpt.apply(lambda cpt: bool(r_wcc.match(cpt)))]
    partitions["sick_encs"] = df[
...
def _calc_stats(df, partitions):
    ...
    stats["wrvu_per_encs"] = stats["ttl_wrvu"] / stats["ttl_encs"]
    ...
    stats["outpt_avg_wrvu_per_pt"] = stats["outpt_ttl_wrvu"] / stats["outpt_num_pts"]
    stats["outpt_num_pts_per_day"] = stats["outpt_num_pts"] / stats["outpt_num_days"]
    stats["outpt_wrvu_per_day"] = stats["outpt_ttl_wrvu"] / stats["outpt_num_days"]
    ...
    stats["outpt_commercial_wrvu"] = partitions["outpt_commercial_encs"].wrvu.sum()
    stats["outpt_commercial_pts"] = len(
        partitions["outpt_commercial_encs"].groupby(["date", "mrn"])
    )
    stats["outpt_commerical_wrvu_per_pt"] = (
        stats["outpt_commercial_wrvu"] / stats["outpt_commercial_pts"]
    )
